...
const t1 = 'a && !!!b';
test(t1, t => {
	t.deepEqual<StringWithIndices[], StringWithIndices[]>(doSplit(t1), [
		{
			characters: 'A',
...
const t2 = 'a && !!==b';
test(t2, t => {
	t.deepEqual<StringWithIndices[], StringWithIndices[]>(doSplit(t2), [
		{
			characters: 'A',
			type: CharacterTypes.variable,
			from: 0,
...
const t3 = 'a && !!==!==b';
test(t3, t => {
	t.deepEqual<StringWithIndices[], StringWithIndices[]>(doSplit(t3), [
		{
			characters: 'A',
			type: CharacterTypes.variable,
			from: 0,
